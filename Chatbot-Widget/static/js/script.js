...
$(document).ready(function() {
    ...
    r = [{"text": "Hola! Soy un Chatbot del Real Jard\u00edn Bot\u00e1nico \00bfen qu\u00e9 te puedo ayudar? Puedes hacerme preguntas sobre las plantas y sus localizaciones dentro del jard\u00edn asi como sus características, y tambi\u00e9n informaciones sobre el jard\u00edn como el plano del jard\u00edn. Tambi\u00e9n puedes iniciar una visita guiada dentro del jard\u00edn."}]
    //setBotResponse(r);
    showBotTyping();
    setTimeout(() => {
        hideBotTyping();
...
function action_trigger() {
    ...
    $.ajax({
        url: 'http://localhost:5008/conversations/user/execute',
        //url: `https://localhost:5055/conversations/${user_id}/execute`,
        type: "POST",
...
function setBotResponse(response) {
    ...
    setTimeout(function() {
        ...
        if (response.length < 1) {
            ...
            var fallbackMsg = "No te he entendido";
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("attachment")) {
                    ...
                    //check if the attachment type is "map"
                    if (response[i].attachment.startsWith("map=") ) {
                        var mapCoord = response[i].attachment;
